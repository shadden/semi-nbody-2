...
void kepler_2D_advance_simple(  CartesianPhaseStateSimple* restrict particle ,double _dt){
	
	const CartesianPhaseStateSimple p1 = *particle;
...
void kepler_2D_advance(  CartesianPhaseState* restrict particle ,double _dt){
	
	const CartesianPhaseState p1 = *particle;
	const double r0 = sqrt(p1.x*p1.x + p1.y*p1.y);
	const double r0i = 1./r0;
	const double v2 =  p1.vx*p1.vx + p1.vy*p1.vy ;
	const double beta = 2.*r0i - v2;
